...
public class GenericWeightMenu
{
    ...
    public void Run()
    {
        while (true)
        {
            ...
            Console.WriteLine("6. Subtract Two Weights");
            Console.WriteLine("7. Subtract Two Weights (Specify Target Unit)");
            Console.WriteLine("8. Divide Two Weights (Get Ratio)");
            Console.WriteLine("9. View All Weight Measurements");
    ...
    {
        ConsoleHelper.WriteSubHeader("SUBTRACT TWO WEIGHTS");
        ...
        try
        {
            ...
            Console.WriteLine("\nSecond Weight (to subtract):");
            double value2 = ConsoleHelper.ReadDouble("Enter value");
            var unit2 = (WeightUnit)GenericUnitSelector.SelectWeightUnit("Select unit");
            var weight2 = new GenericQuantity<WeightUnit>(value2, unit2);

            var result = _service.SubtractMeasurements(weight1, weight2);
    ...
    {
        ConsoleHelper.WriteSubHeader("SUBTRACT TWO WEIGHTS (WITH TARGET UNIT)");
        ...
        try
        {
            ...
            var result = _service.SubtractMeasurementsWithTarget(weight1, weight2, target);
    ...
    {
        ConsoleHelper.WriteSubHeader("DIVIDE TWO WEIGHTS (GET RATIO)");

        try
        {
            Console.WriteLine("\nFirst Weight (dividend):");
            double value1 = ConsoleHelper.ReadDouble("Enter value");
            var unit1 = (WeightUnit)GenericUnitSelector.SelectWeightUnit("Select unit");
            var weight1 = new GenericQuantity<WeightUnit>(value1, unit1);

            Console.WriteLine("\nSecond Weight (divisor):");
            double value2 = ConsoleHelper.ReadDouble("Enter value");
            var unit2 = (WeightUnit)GenericUnitSelector.SelectWeightUnit("Select unit");
            var weight2 = new GenericQuantity<WeightUnit>(value2, unit2);

            double ratio = _service.DivideMeasurements(weight1, weight2);

            ConsoleHelper.WriteSuccess($"\n{weight1} ÷ {weight2} = {ratio:F4}");
        }
        catch (Exception ex)
        {
            ConsoleHelper.WriteError($"Error: {ex.Message}");
